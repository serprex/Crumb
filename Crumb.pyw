...
W.on_draw=lambda:U(A=U((x,__import__("random").randint(100,500))for x in"XY"),L=[V(*a)for a,b in zip(Z,(G(1),O(0,1,1)))][glEnd():O(0,0,0):R(A+9,B+9,A-9,B-9)][R(X+9,Y+9,X-9,Y-9):O(1,0,0):R(X+4,Y+4,X-4,Y-4)]*0)if A else[V(*(L.pop(0),L.pop(0))if a>2 else L[O(1,1,1):2])for a in xrange(len(L)-12,G(3)or-1,-2)][U(A=glEnd()):]
