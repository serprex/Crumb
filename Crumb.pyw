#!/usr/bin/env python
from pyglet.gl import GLfloat as F,glBegin as G,glEnd as D,glLineWidth as N,glVertex2f as V,glColor3f as O,glReadPixels as P,glClear as C,glRecti as R
...
W.on_draw=lambda:U(A=U((x,__import__("random").randint(100,500))for x in"XY"),L=[V(a*4-6,4)for a,a in zip((G(1),O(0,1,1)),Z)][D():O(0,0,0):R(A+9,B+9,A-9,B-9)][R(X+9,Y+9,X-9,Y-9):O(1,0,0):R(X+4,Y+4,X-4,Y-4)]*0)if A else[V(*a>2and (L.pop(0),L.pop(0))or L[O(1,1,1):2])for a in Q(len(L)-12,G(3)or-1,-2)][U(A=D()):]
T.app.run()
